...
class LinearRegression(object):
    ...
    def fit(self, X, y):
        ...
        if self.fit_intercept == True:
            ones = np.ones([len(self._new_X), 1])
            self._new_X = np.concatenate((ones,self._new_X), axis=1)
            
            
        weight = np.linalg.inv(self._new_X.T.dot(self._new_X)).dot(self._new_X.T.dot(y)).flatten()
        
        if self.fit_intercept:
            self._intercept = weight[0]
            self._coef = weight[1:]
        else:
            self._coef = weight
    ...
    def predict(self, X):
        ...
        if self.fit_intercept:
            ones = np.ones(shape=test_X.shape, dtype=np.int8)
            test_X = np.concatenate((ones,test_X), axis=1)
            
            weight = np.concatenate(([self._intercept], self._coef), axis = 0)            
        else:
            weight = self._coef
            
        return test_X.dot(weight)
